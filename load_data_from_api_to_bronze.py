...
def load_data_from_api_to_bronze(list_params, **context):

    start = context['execution_date']
    params = start.strftime('%Y-%m-%d')

    client = InsecureClient(f'http://127.0.0.1:50070', user='user')
    client.makedirs('/DataLake/bronze/out_of_stock/{0}'.format(params))
    ...
    client.delete(os.path.join('/', 'DataLake/bronze/out_of_stock/{0}'.format(params), f'Product[{params}].json'),
                  recursive=False)
    ...
    data = response.json()
    client.write(os.path.join('/', 'DataLake/bronze/out_of_stock/{0}'.format(params), f'Product-{params}.json'),
